...
function DatasetGeneratorWithParameterCalculation(
  startNum = 0,
  endNum = 1,
  bitNumber = 0,
  longNumberRow = 1
...
console.time("test");
const GenArray = DatasetGeneratorWithParameterCalculation(0, 3, 0, 20);
// const GenArray = DatasetGeneratorWithParameterCalculation(
//   -10.5,
//   10.55,
//   0,
//   2000000
...
function test44(of, to, leit) {
  let arr1 = [];
  let sum = 0;
  let average;
  for (let i = 0; i < leit; i++) {
    arr1.push(Math.round(Math.random() * (to + 1)) - of);
  ...
  console.log("Среднее значение: " + (average = Math.round(sum / arr1.length)));
  console.log("Минимальное: " + Math.max.apply(null, arr1));
  console.log("Максимальное: " + Math.min.apply(null, arr1));
}
