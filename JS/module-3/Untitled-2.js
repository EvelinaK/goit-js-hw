...
function randomNumberGenerator(startNum = 0, endNum = 1, bitNumber = 0) {
  ...
  */
  ...
  const random = roundingWithPrecision(
    startNum + Math.random() * (endNum + amendment - startNum),
    bitNumber
  );
  return random;
}
function DatasetGeneratorWithParameterCalculation(
  ...
  let rnd,
    ...
    i = 0;
  for (i = 0; i < 3; i++) {
    rnd = randomNumberGenerator(1, 3, bitNumber);
    Numbers.push(rnd);
    min = min > rnd ? rnd : min;
    max = max < rnd ? rnd : max;
    sum += rnd;
...
// console.log(DatasetGeneratorWithParameterCalculation(0, 1, 0, 2));
// console.log(Math.round(1.005 * 100) / 100);
// console.log(roundingWithPrecision(1.005, 2));
// let startNum = 0,
//   endNum = 2,
//   amendment = 1,
//   bitNumber = 2;
console.log(randomNumberGenerator(4, 10, 2));
